...
public class TicTacToe {
    ...
    public void play() {
        ...
        if (input.length() != 2 || input.charAt(0) < 'A' || input.charAt(0) > 'C' || input.charAt(1) < '1'
                || input.charAt(1) > '3') {
        ...
        if (input.charAt(0) == 'A')
            board[0][(int) (input.charAt(1) - '1')] = 'X';
        else if (input.charAt(0) == 'B')
            board[1][(int) (input.charAt(1) - '1')] = 'X';
        else if (input.charAt(0) == 'C')
            board[2][(int) (input.charAt(1) - '1')] = 'X';
        printBoard();
        if (checkWin()) {
            clearboard();
    ...
    private void printBoard() {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (board[i][j] == '\0')
                    System.out.print(" ");// print a space to make the board look nice
            }
            System.out.println();
        }
    }
    ...
    public boolean checkDraw() {
        ...
        if(checkWin()) return false; //this is not needed as checkwin comes fyrst in the code but for tests it is needed
        clearboard();
        return true;
    }
    ...
    // just for tests, would implement into the play method if needed
